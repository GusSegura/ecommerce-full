...
//         // Aquí redirige o muestra mensaje de éxito
//       },
//       error: (error) => {
...
handleSubmit() {
  ...
  if (this.registerForm.valid) {
    this.authService.register(this.registerForm.value).subscribe({
      next: () => {
        this.router.navigate(['/perfil']); // 👈 navega en éxito
      },
      error: (err) => {
        this.error = err?.error?.message || 'Error en registro'; // 👈 guarda mensaje para el test
      }
    });
  }
}
